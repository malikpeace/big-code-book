...
async function postJSON(path, body) {
  const res = await fetch(path, {
    method: "POST",
    // FAKE_TOKEN + X-Owner are stand-ins for real auth. NOT secure. See chapter 17.
    headers: { "Content-Type": "application/json", "FAKE_TOKEN": "letmein", "X-Owner": "demo" },
...
async function syncState() {
  say("syncing...");
  try {
    const data = await postJSON("/api/sync", state);
    state.revision = data.revision; // chapter 25: remember the server's revision number
    persistState();
    say("synced (revision " + data.revision + ")");
  } catch (err) {
    say("server not running, that is fine, this is a later chapter");
  }
}
